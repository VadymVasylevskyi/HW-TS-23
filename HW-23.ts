...
function delay(ms: number): Promise<string> {
    return new Promise(resolve => setTimeout(() => resolve(`Completed after ${ms} ms`), ms));
}
...
async function handleErrorInPromises() {
    ...
    const promise3 = new Promise<string>((_, reject) => setTimeout(() => reject('Error in Promise 3'), 1500));
...
async function dynamicDelay(numbers: number[]) {
    const promises = numbers.map(num =>
        new Promise<string>(resolve => setTimeout(() => resolve(`Resolved after ${num} ms`), num))
    );
